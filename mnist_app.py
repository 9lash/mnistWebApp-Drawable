...
# import matplotlib.pyplot as plt       # UNCOMMENT FOR LOCAL RUN

# FOR SERVER RUN UNCOMMENT THE FOLLOWING 3 LINES
import matplotlib
matplotlib.use("Agg") 
...
if canvas_result.image_data is not None:
    ...
    data = {
        ...
        "GlobalParameters":  {}
                            ...
                            #     st.write(" Performing Thresholding... " )
                            #     im_og = np.array(image)
                            #     im_gray = cv2.cvtColor(im_og, cv2.COLOR_BGR2GRAY)
                                
                            #     # #guassianblur
                            #     # kernel_size = 5
                            #     # im_gray = cv2.GaussianBlur(im_gray,(kernel_size, kernel_size),0)
                            ...
                            #     st.write(" Finding center of mass...")
                            #     st.write(" Performing affine transform...")
...
if st.button('Predict'):    
    ...
    try:
        ...
        if len(indices) == 1:
            st.write(f' # Digit Prediction: {indices[0]}')
        else:
            st.write("# Digit prediction:")
            for i in indices:
                st.write(i)
